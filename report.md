...
*`1e-4` result is from an earlier 25-epoch run (no RandomCrop) included to illustrate the no-pruning baseline. All other runs: 50 epochs + RandomCrop.*
...
Gate distribution histograms after 50 epochs of training. A healthy pruned model shows a **bimodal** distribution: a spike near 0 (pruned weights) and a cluster near 1 (active weights). The dashed red line marks the 0.01 sparsity threshold.
...
As λ increases, the spike near 0 grows taller and the cluster near 1 shrinks — progressively more weights are fully gated out. At λ=1e-1 virtually the entire distribution collapses to the zero spike.
